...
@Component
@Slf4j
public class UserAddressForForeignKeyConsumer implements AppConsumer {
    ...
    @Override
    public void consume() {
        log.info("UserAddressForForeignKeyConsumer#consume() method invoked.");
        ...
        while (true) {
            ...
            for (ConsumerRecord<?, ?> record : records) {
                ...
                if ("my-topics-06-partitioned.public.user_tbl".equalsIgnoreCase(record.topic())) {
                    ...
                    userSet.add(address.getUserId());
                } else {
                    log.error("Invalid for topic {{}}", record.topic());
                }
            }
            final UniqueUserAddressIds uniqueUserAddressIds = UniqueUserAddressIds.builder().userIds(userSet).addressIds(new HashSet<Long>()).build();
